data "template_file" "kubeconfig_kube_controller_manager" {
  ...
  vars = {
    ...
    kube_user_certificate          = base64encode(tls_locally_signed_cert.kube_controller_manager.cert_pem)
    kube_user_private_key          = base64encode(tls_private_key.kube_controller_manager.private_key_pem)
  }
}
